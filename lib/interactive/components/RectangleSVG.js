...
//tolik
var _shape = require("../components/ClickableCircle");
...
var Rectangle = function (_Component) {
...

  {
    ...
    value: function drawOnCanvas(ctx, moreProps) {
      ...
      ctx.fillStyle = "rgba(47, 189, 47, .2)";
      ctx.fillRect(x1, y1, width, height);
      ctx.strokeRect(x1, y1, width, height);

      const offset = 4;
			const markerWidth = 2 * offset + 1;
			const markerHeight = 2 * -offset - 1;

      // top left marker rectangle
      ctx.beginPath();
      ctx.fillStyle = "rgba(47, 189, 47, .6)";
			ctx.moveTo(x1, y1);
      ctx.fillRect(x1 - offset, y1 + offset, markerWidth, markerHeight);

      // bottom left marker rectangle
      ctx.beginPath();
      ctx.fillStyle = "rgba(47, 189, 47, .6)";
			ctx.moveTo(x1, y2);
			ctx.fillRect(x1 - offset, y2 + offset, markerWidth, markerHeight);

      // bottom right marker rectangle
      ctx.beginPath();
      ctx.fillStyle = "rgba(47, 189, 47, .6)";
			ctx.moveTo(x2, y2);
      ctx.fillRect(x2 - offset, y2 + offset, markerWidth, markerHeight);

      // top right marker rectangle
      ctx.beginPath();
			ctx.moveTo(x2, y1);
			ctx.fillRect(x2 - offset, y1 + offset, markerWidth, markerHeight);
    }
    },
    ...
    key: "render",
      ...
      var _props6 = this.props,
        ...
        onUnHover = _props6.onUnHover;
         ...
         var _helper2 = helper(this.props, tolMore),
          x1 = _helper2 ? _helper2.x1 : 0,
          y1 = _helper2 ? _helper2.y1 : 0,
          x2 = _helper2 ? _helper2.x2 : 0,
          y2 = _helper2 ? _helper2.y2 : 0;
...
const path = (_ref)=>{
  ...
  return `M${x1} ${y1} L ${x1 + w} ${y1} L ${x1 + w} ${y1 - h} L ${x1} ${y1 - h}`;

  // return "M" + x1 + " " + y1 + " " + ("L" + (x1 + halfWidth) + " " + (y1 + halfWidth) + " ") + ("L" + (x1 + bottomWidth) + " " + (y1 + halfWidth) + " ") + ("L" + (x1 + bottomWidth) + " " + (y1 + height) + " ") + ("L" + (x1 - bottomWidth) + " " + (y1 + height) + " ") + ("L" + (x1 - bottomWidth) + " " + (y1 + halfWidth) + " ") + ("L" + (x1 - halfWidth) + " " + (y1 + halfWidth) + " ") + "Z";
